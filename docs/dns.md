...
| --------------------------- | ----- | ------------------------------------------------------------------------ | ------------------------------------------------------------ |
| `xn--80aaxzavh.xn--p1ai.`   | ALIAS/A (или CNAME через CDN) | `<bucket>.website.yandexcloud.net` или CDN-домен | Лендинг (Yandex Object Storage website endpoint) |
| `app.xn--80aaxzavh.xn--p1ai.` | CNAME | `<bucket>.website.yandexcloud.net` или CDN-домен                         | Личный кабинет (тот же bucket или отдельный)                 |
| `api.xn--80aaxzavh.xn--p1ai.` | CNAME | `<function-id>.apigw.yandexcloud.net`                                   | API Gateway, перед которым Cloud Function                    |
...
- `xn--80aaxzavh.xn--p1ai`
- `*.xn--80aaxzavh.xn--p1ai`

## Публикация API

API-шлюз Yandex Cloud API Gateway, маршрут `/*` → Cloud Function `bezbrehni-api`. Включить CORS. Привязать custom domain `api.xn--80aaxzavh.xn--p1ai` с сертификатом из Certificate Manager.
...
1. В `.env` выставить `API_URL=https://xn--80aaxzavh.xn--p1ai/api`.
